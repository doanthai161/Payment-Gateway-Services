...
class Transaction(UUIDModel, TimeStampedModel):
    ...
    reference_id = models.CharField(
        max_length=100, unique=True, help_text=_("Mã giao dịch tham chiếu (Mã đơn hàng nội bộ)")
    )
    ...
    provider = models.CharField(
        max_length=50, help_text=_("Nhà cung cấp thanh toán (e.g., vnpay, momo, stripe, banking)")
    )
    payment_method = models.CharField(
        max_length=50, blank=True, null=True, help_text=_("Phương thức thanh toán cụ thể")
    )
    ...
    gateway_transaction_id = models.CharField(
        max_length=255, blank=True, null=True, db_index=True, help_text=_("Mã giao dịch từ Gateway trả về")
    )
